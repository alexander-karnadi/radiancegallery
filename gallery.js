var fs = require('fs'), exif = require('./exif.js'), walk = require('walk'), util = require('util'), path = require('path'), im = require('imagemagick');
...
var gallery = {
	...
	readFiles : function(params, cb) {
		var files = [], directoryPath = (this.static) ? this.static + "/" + this.directory : this.directory, me = this;
	...
	buildAlbums : function(files, cb) {
		...
		for (var i = 0; i < files.length; i++) {
			...
			var file = files[i], dirs = file.rootDir.split("/"), dirHashKey = "", curAlbum = albums;
			// reset current album to root at each new file
			...
			for (var j = 0; j < dirs.length && dirs[0] !== ""; j++) {
				...
				if (!dirHash.hasOwnProperty(dirHashKey)) {
					...
					// reconstruct the current path with the path slashes
		...
		function _recurseOverAlbums(al) {
			...
			if (!al.thumb) {
				...
				// only set this album's thumbanil if not already done in info.json
			}
		...
		function _buildThumbnails(album) {
			var photoChildren = album.photos, albumChildren = album.albums;
	...
	init : function(params, cb) {
		var me = this, directory = params.directory, staticDir = params.static;
		...
		if (!directory)
			throw new Error('`directory` is a required parameter');
		...
		if (directory.charAt(directory.length - 1) === "/") {
			...
			// yes length-1 - .lenght is the full string remember
		}
		if (staticDir.charAt(staticDir.length - 1) === "/") {
			staticDir.substring(0, staticDir.length - 1);
			// yes length-1 - .lenght is the full string remember
		}
	...
	getAlbum : function(params, cb) {
		var album = this.album, albumPath = params.album;
	...
	afterGettingItem : function(err, data, cb) {
		...
		var back = data.back = breadcrumb.slice(0, item.path.split("/").length - 1).join("/");
		// figure out up a level's URL
	...
	middleware : function(options) {
		...
		return function(req, res, next) {
			var url = req.url, rootURL = gallery.rootURL, params = req.params, requestParams = {}, image = false;
				...
				if (thumbTest.test(url)) {
					...
					if (me.imageCache[imagePath]) {
						...
						lwip.open(imagePath, function(err, image) {
							...
							//image.batch().scale(0.75);
							image.resize(260, function(err, resizeImg) {
								image.toBuffer('jpg', function(err, buffer) {
									res.contentType('image/jpg');
									res.end(buffer, 'binary');
									me.imageCache[imagePath] = buffer;
								});
							});
						});
			...
			if (url && url !== "") {
				var filepath = url.trim(), isFile = /\b.(jpg|bmp|jpeg|gif|png|tif)\b$/;
